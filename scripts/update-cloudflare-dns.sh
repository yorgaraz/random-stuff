#!/usr/bin/env bash
# Original: https://raw.githubusercontent.com/fire1ce/DDNS-Cloudflare-Bash/refs/heads/main/update-cloudflare-dns.sh
...
if [ "${what_ip}" == "external" ]; then
  iface_arg=""
  if [[ -n "$discovery_iface" && "$discovery_iface" != "false" ]]; then
    echo "==> Using specified discovery interface: $discovery_iface"
    iface_arg="--interface $discovery_iface"
  fi

  ip=$(curl $iface_arg -4 -s -X GET https://checkip.amazonaws.com --max-time 10)
